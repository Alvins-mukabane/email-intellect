...
export function DashboardIndex({ emails }: DashboardIndexProps) {
  const tasks = emails
    .flatMap((email) => {
      try {
        return email.action_items ? JSON.parse(email.action_items) : [];
      } catch {
        return [];
      }
    })
  ...
  const opportunities = emails.filter((email) => email.is_opportunity);
